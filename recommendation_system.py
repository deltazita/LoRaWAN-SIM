...
DEFAULT_MODEL_DIR = SCRIPT_DIR / "ML"
...
def perl_args_from_config(config: Dict[str, Any]) -> List[str]:
    """Convert a configuration to the positional argument order expected by Perl."""
    cfg = normalize_config(config)
    return [str(cfg[key]) for key in CANONICAL_KEYS]
...
def run_lorawan_simulator(config: Dict[str, Any], perl_script: Optional[str] = None) -> Dict[str, Any]:
    ...
    (script_path.parent / "tmp").mkdir(parents=True, exist_ok=True)
    cmd = ["perl", str(script_path), *perl_args_from_config(config)]
    print("Running LoRaWAN.pl once for baseline.")
    ...
    if completed.returncode != 0:
        raise SimulationError(
            f"LoRaWAN.pl failed with return code {completed.returncode}.\n"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run LoRaWAN.pl once for the baseline configuration, then use the classifier for PRR/PDR and the regressor for energy recommendations."
    )
    ...
    parser.add_argument("--perl-script", type=str, default=str(DEFAULT_PERL_SCRIPT), help="Path to LoRaWAN.pl. Defaults to ./LoRaWAN.pl next to this script.")
